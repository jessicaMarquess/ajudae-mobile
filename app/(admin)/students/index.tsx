...
import { useDeleteStudent, useGetStudents } from "@/src/hooks/useQueries";
...
export default function StudentsListScreen() {
  ...
  const { data, isLoading, refetch, isFetching } = useGetStudents(
    currentPage,
    10
  );
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteStudentMutation.mutateAsync(selectedDeleteId);
  ...
  const renderStudentItem = ({ item }: any) => (
    <View style={styles.studentCard}>
      <View style={styles.studentInfo}>
        ...
        {item.registration && (
          <Text style={styles.studentReg}>Matrícula: {item.registration}</Text>
        )}
        {item.course && (
          <Text style={styles.studentCourse}>Curso: {item.course}</Text>
        )}
      </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity
        ...
        disabled={!isTeacher}
      >
        <Text style={styles.createButtonText}>+ Novo Estudante</Text>
      </TouchableOpacity>
